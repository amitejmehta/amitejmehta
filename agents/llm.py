...
class LLM:
    ...
    @messages.setter
    def messages(self, messages: List[Dict[str, Any]]):
...
def parse_signature(func: Callable) -> dict:
    """Get parameters and required parameters from a function signature."""
    if not inspect.iscoroutinefunction(func):
        raise ValueError(f"Tool {func.__name__} must be a async function.")
...
def to_json(func: Callable) -> dict:
