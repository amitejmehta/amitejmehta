...
class PositionalEncoding(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.denominator = torch.exp(torch.arange(
            0, self.d_model, 2) * -(math.log(10000.0) / self.d_model))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, args: ModelArgs, attn_mask=False):
        ...
        self.heads = nn.ModuleList(
            [SelfAttention(args, self.attn_mask) for _ in range(args.num_heads)])
...
class EncoderLayer(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.ff = PositionWiseFeedForward(
            args.d_model, args.d_ff, args.dropout)
